...
def name_to_id(region_name):
    '''
    Return region id for a given region name
...
def id_to_name(region_id):
    '''
    Return region name for a given woeid

    region_id: region woeid
...
def is_country(region_id):
    '''
    Return whether or not a region is a country
    '''

    for region in regions:
        if region['woeid'] == region_id and region['placeType']['code'] == 12:
            return True
    
    return False

def is_town(region_id):
    '''
    Return whether or not a region is a town

    Note: the code for denoting towns is 7, but there are some unknown places that we can consider a town
          that have the placetype code 9 and 22. 

    Another note: I think there is a better way to do this but my mind is so used to think about for loops
          for everything. Sorry.
    '''

    for region in regions:
        if region['woeid'] == region_id and (
            region['placeType']['code'] == 7 or
            region['placeType']['code'] == 9 or
            region['placeType']['code'] == 22):
            return True
    
    return False

def get_country_of_town(region_id):
    '''
    Return the country of a given place if this place is a town

    Note: it depends on is_town() function
    '''

    if not is_town(region_id):
        raise ValueError("region_id is not a town")
    else:
        for region in regions:
            if region['woeid'] == region_id:
...
def get_countries_list():
    ...
    for region in regions:
        ...
        if is_country(region_id):
            countries.append(region_id)
...
def get_towns_list():
    ...
    for region in regions:
        ...
        if is_town(region_id):
            towns.append(region_id)
